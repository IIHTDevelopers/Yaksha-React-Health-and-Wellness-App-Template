...
describe('boundary', () => {
    ...
    test('MealPlansComponent boundary form renders for adding a new meal plan', async () => {
        ...
        await waitFor(() => expect(screen.getByLabelText('Meal Type:')).toBeInTheDocument());
        await waitFor(() => expect(screen.getByLabelText('Description:')).toBeInTheDocument());
    });

    test('MealPlansComponent boundary can add a meal to the plan before submitting', async () => {
        render(<MealPlans />);
        ...
        fireEvent.change(screen.getByLabelText('Meal Type:'), { target: { value: 'Dinner' } });
        fireEvent.change(screen.getByLabelText('Description:'), { target: { value: 'Grilled Salmon' } });
        fireEvent.click(screen.getByText('Add Meal to Plan'));
    });
