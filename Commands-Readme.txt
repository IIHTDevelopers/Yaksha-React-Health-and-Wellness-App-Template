* You can follow series of command to setup Angular environment once you are in your project-name folder:
	...
	npm run start -> To compile and deploy the project in browser. You can press <Ctrl> key while clicking on localhost:4000 to open project in browser -> takes 2 to 3 min

	npm run start:db -> As we are using a json server to mimic our db.json file as a database. So, this command is useful to start a json server.
